...
if "win" in sys.platform:
    ...
    # sys.path.append(r'/mnt/usmidet/projects/DMA-DATA/8-Users/perception_validation/utils_pv')
    # from data_base_update import upload_event_to_table
...
class StreamCheckMcip():
    ...
    def __init__(self):
        ...
        self._headers['overview_stream_check'] = ['log_path', 'log_name', 'CAN_busID_2111', 'CAN_busID_2112',
                                                  'CAN_busID_2211', 'CAN_busID_2212', 'CAN_busID_2102', 'CAN_busID_2103',
                                                  'CAN_busID_2120', 'busID_1001060_source_23_stream_18',
                                                  'busID_1001060_source_23_stream_19',
                                                  'busID_1001060_source_23_stream_80',
                                                  'busID_1001060_source_23_stream_81',
                                                  'busID_1001060_source_23_stream_85',
                                                  'busID_1001060_source_23_stream_117',
                                                  'busID_1001060_source_104_stream_37', 'swiftNav_msgID_522',
                                                  'swiftNav_msgID_529', 'swiftNav_msgID_535']
    ...
    def _string_replace(self, file_name):

        sub_string_list = ['_bus_', '_deb_', '_ref_']
        boolean_substring = list(map(file_name.__contains__,
                                 sub_string_list))
        ...
        path_list = [file_name.replace(*iter_tuple)
    ...
    def kpi_sheet_generation(self, output_excel_sheet, **kwargs):
        ...
        if "lin" in sys.platform:
            ...
            from eventExtraction.utils\
                .utils_generic import create_mysql_engine_fn, update_sql_table
        ...
        def _df_from_list_dicts(excel_out_path, sheet_name, col_name):
            ...
            req_cols_data_va_tool_row_wise = \
                req_cols_data_va_tool.to_dict(orient='records')
            req_cols_data_va_tool_row_wise = [

                {key: [val] for key, val in list_item.items()}

                for list_item in req_cols_data_va_tool_row_wise
            ]

            req_cols_data = [eval(item) for item in req_cols_data]

            req_df_lists_eth = [
                pd.DataFrame({**row_dict, **{key.strip(): val
                                             if isinstance(val, list)
                                             else [val]
                                             for key, val in list_item[0].items()
                                             # if key.strip() in req_streams_eth
                                             }, })
                for list_item, row_dict in
                zip(req_cols_data,
                    req_cols_data_va_tool_row_wise)]

            req_df_eth = pd.concat(req_df_lists_eth,
                                   axis=0, ignore_index=True,).fillna(
                                       'not_available')

            req_df_lists_can = [
                pd.DataFrame({**row_dict, **{'CAN_busID_'+str(key): val
                                             if isinstance(val, list)
                                             else [val]
            ...
            req_df_lists_swift_nav = [
                pd.DataFrame({**row_dict, **{'swiftNav_msgID_'+str(key): val
                                             if isinstance(val, list)
                                             else [val]
            ...
            out_df = reduce(lambda left, right: pd.merge(left, right,
                                                         on=['log_path',
                                                             'log_name'],
                                                         how='outer'),
                            [req_cols_data_va_tool,
                             req_df_can,
                             req_df_eth,
                             req_df_swift_nav,]
                            )

            out_df['base_logname'] = out_df['log_name'].apply(
                lambda x: '_'.join(x.split('.')[0].split('_')[:-1]))
        ...
        out_df = pd.DataFrame(columns=self._headers['overview_stream_check'])

        out_df_partial = _df_from_list_dicts(
            excel_out_path, sheet_name, col_name)

        out_df[out_df_partial.columns] = out_df_partial
        ...
        if 'lin' in sys.platform:
            ...
            print('\n#####################################\n',
                  'CAUTION : ABOUT TO WRITE TO A SQL TABLE',
                  '\n#####################################\n',)

            sql_engine_object = create_mysql_engine_fn()
            table_name = 'DMA_MCIP_ALL_STREAM_CHECK_01'
            sql_engine_object_url = update_sql_table(sql_engine_object,
                                                     out_df,
                                                     table_name)
    ...
    def run(self, file_name, **kwargs):
        ...
        **kwargs : TYPE
        ...
        log_name_mf4 = log_name.split('.')[0]

        if '2-Sim' in log_path:

            log_path_mf4 = log_path.replace('2-Sim', '1-Raw', 1)
        else:
            log_path_mf4 = log_path

        file_name = os.path.join(log_path_mf4, log_name_mf4+'.MF4')
        ...
        if not os.path.isfile(self._save_mcip_stream_map_path):
            ...
            req_map_df, req_can_map_df = self._process_stream_check(mcip_obj)
        ...
        if 'is_stream_check_dict' in kwargs.keys():

            if (kwargs['is_stream_check_dict'].lower() in
                ['true', '1', 't', 'y',
                 'yes', 'yeah', 'yup', 'certainly', 'uh-huh']):

                is_stream_check_dict = True
            else:
                is_stream_check_dict = False
        else:
            is_stream_check_dict = False

        if 'is_stream_check_dict' in kwargs and is_stream_check_dict:
...
if __name__ == "__main__":
    ...
    if "win" in sys.platform:
        ...
        kwargs['run_mode'] = '0000'
        kwargs['is_stream_check_dict'] = '0'
        is_pickle = False
        ...
        if is_remote_windows:
            ...
            if not is_parallel_run:
                ...
                yy = [out_item['output_sheet'] for out_item in final_out]
                df_combined = pd.concat(yy, axis=0)
                output_excel_sheet = \
                    os.path.join(
                        req_dir, 'test_stream_check_17092025_1730.xlsx')
                df_combined.to_excel(output_excel_sheet,
                                     sheet_name='output_sheet',
                                     index=False)
                kpi_df_01 = class_obj.kpi_sheet_generation(output_excel_sheet)
        end_time = time.time()
