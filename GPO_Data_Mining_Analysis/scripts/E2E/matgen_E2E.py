...
class MatgenE2e():
    ...
    def run(self, file_name, **kwargs):
        ...
        **kwargs : TYPE
        ...
        from eventExtraction.utils\
            .mat73 import loadmat, savemat

        # from eventExtraction.utils\
        #     .pymatreader import read_mat as loadmat
        ...
        if run_mode in [1606, ]:
            ...
            if run_mode == 1606:
                ...
                req_mat_types = ['_b05', '_p01', ]

            for req_mat_type in req_mat_types:

                out_mat_file_name = re.sub(compiled_strings, req_mat_type,
                ...
                mat_version = matfile_version(existing_mat_full_path)[0]
                if mat_version < 2:

                    data_mat = load_mat_scipy(existing_mat_full_path,
                                              **kwargs_load_mat_scipy
                                              )
                else:
                    data_mat = loadmat(existing_mat_full_path)

                data = merge_dicts(data, data_mat)

                print('\n&&&&&&&& 577 &&&&&&&&&&\n',
                      os.path.isfile(existing_mat_full_path),
                      '\n&&&&&&&&&&&&&&&&&&&&&&&&&\n')
        ...
        if (os.path.isfile(save_mat_full_path)
            ...
            if not run_mode in [1606, ]:

                mat_version2 = matfile_version(existing_mat_full_path)[0]
                ...
                if mat_version2 < 2:
                    ...
                    if os.path.isfile(existing_mat_full_path):
                        ...
                        data = loadmat(existing_mat_full_path)
                    elif os.path.isfile(save_mat_full_path):

                        data = loadmat(save_mat_full_path)
        ...
        if matfile_version(existing_mat_full_path)[0] < 2:
            if os.path.isfile(save_mat_full_path):
...
if __name__ == "__main__":
    ...
    if "win" in sys.platform:
        ...
        file_list_path = os.path.join(req_dir, 'file_list_e2e.txt')
