...
class Comparehpcnexusvisionagtgrid:
    """
    ...
    def __init__(self):
        ...
        self._headers['autogt_matched_data'] = ['log_path',
                                                'log_name',
        ...
        self._headers['vehicle_matched_objects'] = ['log_path',
    ...
    def KPI_statistics(self, joined_df):
        ...
        return kpi_pos_object_table, kpi_vel_object_table, kpi_dem_object_table, kpi_heading_object_table
    ...
    def grid_output(self, joined_df, orphans, output_excel_path):
        import shutil
        if "lin" in sys.platform:
        ...
        for grid_pos in joined_grid_df['cell'].unique():
            ...
            lat_velocity_sheet[grid_pos[0]].value = str(lat_vel_pos_err_mean) + '\n' + str(
                ...
                number_of_samples)
            # long_position_sheet[grid_pos[0]].value = long_pos_err_median
            # lat_position_sheet[grid_pos[0]].value = lat_pos_err_median
            # long_velocity_sheet[grid_pos[0]].value = long_vel_err_median
            # lat_velocity_sheet[grid_pos[0]].value = lat_vel_err_median
        wb.save(grid_excel_path)

    def runKPI(self, MatchedGT_df, MtachedJLBUX_df, orphans, output_excel_path):
        ...
        self.grid_output(joined_df, orphans, output_excel_path)
        kpi_pos_object_table, kpi_vel_object_table, kpi_dem_object_table, kpi_heading_object_table = self.KPI_statistics(
            joined_df)
        ...
        writer = pd.ExcelWriter('LIDAR_Thunder_KPI_' + str(timestr) + '.xlsx')
    ...
    def kpi_sheet_generation(self, output_excel_sheet):
        ...
        matched_agt_all_logs = []
        matched_JLBUX_all_logs = []
        os.chdir(output_excel_path)
        ...
        for i in all_excels:
            try:
                print('loading', i)
                gt_data = pd.read_excel(i, sheet_name='autogt_matched_data')
                matched_data = pd.read_excel(i, sheet_name='vehicle_matched_objects')
        ...
        gt_data_df = pd.concat(matched_agt_all_logs)
        JLBUX_data_df = pd.concat(matched_JLBUX_all_logs)
        # all_orphans_df = pd.concat(orphans_all_logs)
        ...
        if len(gt_data_df) == 0:
            ...
            all_orphans_df = pd.DataFrame(columns=gt_data_df.columns)
            all_orphans_df.rename(columns={'Lateral_position': 'Lateral_position_GT'}, inplace=True)
            all_orphans_df.rename(columns={'Longitudinal_position': 'Longitudinal_position_GT'}, inplace=True)
            self.runKPI(gt_data_df, JLBUX_data_df, all_orphans_df, output_excel_path)
    ...
    def timesync_fus(self, data, log_type):
        try:
            ...
            if max(fusion_measurement_age_at_tx_us) > 1000000:
    ...
    def run(self, file_name, **kwargs):
        ...
        try:
            ...
            dfMatchedlog_df,dfMatchedGT_df, orphans_veh_df, orphans_autogt_df = abcompare.run(autoGT_df,vehicle_vision_raw_df)

            print("Done correlation")

            dfMatchedGT_df.insert(0, 'log_path', picklefile_path)
            dfMatchedGT_df.insert(1, 'log_name', baselogname)

            dfMatchedlog_df.insert(0, 'log_path', logPath)
            dfMatchedlog_df.insert(1, 'log_name', logName)

            # orphans_df.insert(0, 'log_path', logPath)
            # orphans_df.insert(1, 'log_name', logName)

            orphans_autogt_df.insert(0, 'log_path', picklefile_path)
            orphans_autogt_df.insert(1, 'log_name', baselogname)

            out = dict()
            # Sheet name needs to match with sheet_name in self._headers
            # Make sure header column count matches with output column count
            out['autogt_matched_data'] = np.array(dfMatchedGT_df.values)
            out['vehicle_matched_objects'] = np.array(dfMatchedlog_df.values)
...
if __name__ == '__main__':
    kwargs = dict()
    # path = r"/mnt/usmidet/projects/FCA-CADM/11-Development/8-DMA_Reports/JL_BUX/AB_compare_script_testing_lidar/FCAWL_20231011_JlBuxWilma_ChkOut_80p00p25p01_TRY_TRY_AZ_RM_160336_038.mat"
    # file_name = os.path.join(path, 'FCAWL_20231011_JlBuxWilma_ChkOut_80p00p25p01_TRY_TRY_AZ_RM_160336_038.mat')
    # file_name = r'/mnt/usmidet/projects/FCA-CADM/11-Development/8-DMA_Reports/JL_BUX/AB_compare_script_testing_lidar/FCAWL_20231011_JlBuxWilma_ChkOut_80p00p25p01_TRY_TRY_AZ_RM_160336_038.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Trimble_Parser\input.hdf5'\
    # file_name  = r'C:\Users\pjp8lm\Desktop\Thunder\pickle_files\TNDR1_DRUK_20240229_230153_WDC5_bus_0000.mat'
    ...
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\RESIM_logs\TNDR1_DRUK_20240627_211849_WDC5_rA24000124279638_dma_0032.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\Golden_dataset\TNDR1_DRUK_20240530_164859_WDC5_dma_0006.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\Golden_dataset\RESIM_logs\TNDR1_DRUK_20240524_225225_WDC5_rFLR240008243301_r4SRR240011243301_rM05_rVs05070011_rA24010124369190_dma_0010.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\Golden_dataset\RESIM_logs\TNDR1_DRUK_20240701_195955_WDC5_dma_0002.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\Golden_dataset\RESIM_logs\TNDR1_DRUK_20240524_225225_WDC5_dma_0010.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\Golden_dataset\RESIM_logs\TNDR1_DRUK_20240524_225225_WDC5_dma_0010.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\Golden_dataset\TNDR1_DRUK_20240530_155144_WDC5_dma_0052.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\Golden_dataset\TNDR1_ASHN_20240312_030129_WDC5_dma_0016.mat'
    # file_name = r'C:\Users\pjp8lm\Desktop\Thunder\Latest_of_logs_agt_tool\AEB_SCP_event\TNDR1_DRUK_20240729_113423_WDC5_dma_0006.mat'

    Test = Comparehpcnexusvisionagtgrid()
    headers = Test.get_headers()
    cell = Test.run(file_name, **kwargs)
    # print(cell)

    autogt_matched_data = pd.DataFrame(data =cell['autogt_matched_data'],columns=headers['autogt_matched_data'])
    vehicle_matched_objects = pd.DataFrame(data =cell['vehicle_matched_objects'], columns=headers['vehicle_matched_objects'])
    # orphans = pd.DataFrame(data =cell['orphans'] ,columns=headers['orphans'])


    # autogt_matched_data = pd.read_csv(
    #     r'C:\Users\pjp8lm\Desktop\Thunder\AGT\macthed_autogt_Velocity_ref_position_vel_IOU.csv')
    # del autogt_matched_data['Unnamed: 0']
    # vehicle_matched_objects = pd.read_csv(
    #     r'C:\Users\pjp8lm\Desktop\Thunder\AGT\macthed_vehicle_Velocity_ref_position_vel_IOU.csv')
    # del vehicle_matched_objects['Unnamed: 0']

    path, name = os.path.split(file_name)
    current_apth = os.chdir(path)
    writer = pd.ExcelWriter('Thunder_tracker_KPI' + '.xlsx')
    autogt_matched_data.to_excel(writer, "autogt_matched_data", index=False)
    vehicle_matched_objects.to_excel(writer, "vehicle_matched_objects", index=False)
