...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        for shape in reversed([s for s in self.shapes if self.isVisible(s)]):
            ...
            index = shape.nearestVertex(
                pos, self.epsilon / self.scale if self.scale > 1 else self.epsilon)
    ...
    def boundedMoveShape(self, shape, pos):
        if shape.isRotated and self.canOutOfBounding:
            ...
            if dc.x() >= self.pixmap.width():
                dp += QPointF(min(0, self.pixmap.width() -
                              1 - dc.x()), 0)  # TODO
    ...
    def paintEvent(self, event):
        ...
        if (self.drawing() or self.continueDrawing()) and not self.prevPoint.isNull() and not self.outOfPixmap(self.prevPoint):
            ...
            # TODO : limit pen width
            p.setPen(QPen(QColor(255, 255, 255), 1/self.scale))

            p.drawLine(int(self.prevPoint.x()),
                       0,
