...
class DecoderBase(object, metaclass=ABCMeta):
    ...
    def _extract_signal_bits(cls, signal: Signal, data: np.ndarray) -> np.ndarray:
        ...
        if reduced_data.size == 0:
            warnings.warn("No data found for signal {}".format(
                signal), MissingDataWarning)
            return np.empty(shape=(data.shape[0], 0))

        # Determine how to read the data depending on the endianness.
        if signal.is_little_endian:
            reduced_data_bits = np.unpackbits(
                reduced_data, axis=1, bitorder="little")
        else:
            reduced_data_bits = np.unpackbits(
                reduced_data, axis=1, bitorder="big")

        # Extract the signal bits, ignoring the surrounding data.
        subbyte_start_position = start_bit % 8
        bit_data_trail = reduced_data_bits[:,
                                           subbyte_start_position:subbyte_start_position + signal.size]
    ...
    def _decode_signal_raw(cls, signal: Signal, data: np.ndarray) -> Optional[np.ndarray]:
        ...
        if len(signal_data.shape) == 1:
            ...
            expanded_data = np.empty(
                (signal_data.shape[0], next_size), dtype=np.uint8)
        ...
        try:
            ...
            warnings.warn("Could not shape data for {}".format(
                signal), DataSizeMismatchWarning)
        ...
        try:
            ...
            print(
                f'%%%%%%%%%%%%%%%%%%%%% Exception \n{e} occured in signal\n', signal, )
            data_return = np.empty(shape=(new_shape[0], 0))
    ...
    def _handle_float_signal(signal: Signal, data: np.ndarray) -> np.ndarray:
        if signal.size == 32:
            ...
            raise RuntimeError(
                "Signal should be decoded as float, but is not 32 or 64 bits wide")
    ...
    def _handle_integer_signal(signal: Signal, data: np.ndarray) -> np.ndarray:
        ...
        if signal.is_signed:
            ...
            msb_mask = data.dtype.type(
                np.iinfo(data.dtype.type).max) & ~mask_signal_select
