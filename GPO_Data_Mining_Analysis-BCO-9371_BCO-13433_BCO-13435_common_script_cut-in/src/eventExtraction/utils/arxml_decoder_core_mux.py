...
def convert_to_bitstruct_string(datatype_dict, E2E_Header=True):
    ...
    if E2E_Header:
        ...
        E2E_header_signals_regex_2 = "^LRRF_Det_.+?_[0-9]+"
        E2E_header_signals_regex_3 = "LRRF_A._Header_Timestamps_.+"
        E2E_indices = [idx for idx, signal in enumerate(signal_names)
                       if re.match(E2E_header_signals_regex, signal)
                       or re.match(E2E_header_signals_regex_2, signal)
                       or re.match(E2E_header_signals_regex_3, signal)]
...
def decode_payload(payload_numpy_array, timestamps, msg_decode_schema):
    ...
    pdu_id, dlc, not_used = decode_autosar_pdu_header(sample_payload_header, 0)
...
def decode_autosar_pdu_header(data_bytes, udp_length):
    ...
    mux_len = udp_length - UDP_HEADER_SIZE_BYTES - \
        AUTOSAR_PDU_HEADER_SIZE_BYTES - DLC
    return PDU_ID, DLC, mux_len


def decode_autosar_mux(x, append_list):
    udp_length = x["UDP_Length"]
    payload = x["UDP_Payload"]
    demux_recursive(payload, udp_length, x, append_list)
    return 0


def demux_recursive(payload, mux_len, mux_series, append_list):
    if len(payload) == 0:
        return 0
    else:
        PDU_ID, DLC, mux_len = decode_autosar_pdu_header(payload, mux_len)
        payload_demux = payload[0:(DLC + AUTOSAR_PDU_HEADER_SIZE_BYTES)]
        series_copy = mux_series.copy()
        series_copy["autosar_pdu"] = PDU_ID
        series_copy["DLC"] = DLC
        series_copy["UDP_Payload"] = payload_demux
        append_list.append(series_copy)
        payload = payload[(DLC + AUTOSAR_PDU_HEADER_SIZE_BYTES):]
        demux_recursive(payload, mux_len + UDP_HEADER_SIZE_BYTES,
                        mux_series, append_list)
...
def decode_eth_channel_by_arxml(dataframe, arxml_path):
    ...
    autosar_pdu_filter_df['UDP_Payload'] = autosar_pdu_filter_df.apply(
        lambda x: x["UDP_Payload"][0:(x["UDP_Length"] - UDP_HEADER_SIZE_BYTES)], axis=1)
    # Add a column "mux_len" for DLC to UDP len check
    autosar_pdu_filter_df["autosar_pdu"], autosar_pdu_filter_df["DLC"], autosar_pdu_filter_df["mux_len"] = zip(
        *autosar_pdu_filter_df.apply(
            lambda x: decode_autosar_pdu_header(x["UDP_Payload"], x["UDP_Length"]), axis=1))
    # If non-zero, filter and send through mux logic
    autosar_pdu_filter_df_mux = autosar_pdu_filter_df[autosar_pdu_filter_df["mux_len"] > 0]
    if not autosar_pdu_filter_df_mux.empty:
        # If there are muxed messages, then apply demuxing
        autosar_pdu_filter_df_mux_unpacked = []
        autosar_pdu_filter_df_mux.apply(lambda x: decode_autosar_mux(
            x, autosar_pdu_filter_df_mux_unpacked), axis=1)  # beware of mutable lists
        autosar_pdu_filter_df_mux_unpacked = pandas.DataFrame(
            autosar_pdu_filter_df_mux_unpacked)
        # Delete original entries from df without mux logic
        autosar_pdu_filter_df = autosar_pdu_filter_df.drop(
            autosar_pdu_filter_df_mux.index)
        # Append mux df and re-sort by timestamp
        autosar_pdu_filter_df = pandas.concat(
            [autosar_pdu_filter_df, autosar_pdu_filter_df_mux_unpacked])
        autosar_pdu_filter_df = autosar_pdu_filter_df.sort_values(
            ["timestamps", "autosar_pdu"])
        autosar_pdu_filter_df = autosar_pdu_filter_df.reset_index().drop(columns=[
            "index"])
    ...
    pdu_decode["ETH_TIMING"] = dict()
    pdu_decode["ETH_TIMING"]["time"] = numpy.array(
        autosar_pdu_filter_df["timestamps"])
    # .to_list()
    pdu_decode["ETH_TIMING"]["pdu_id"] = autosar_pdu_filter_df["autosar_pdu"].to_numpy()
