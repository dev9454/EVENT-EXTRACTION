...
class FibexParser(AbstractParser):
    ...
    def finalize_ecus(self):
        ...
        for ecu_name, data in self.__ecu_data__.items():
            ...
            if len(tmp_ctrllist) != len(ctrllist) + len(ecu_data[1]):
                print(
                    f"INTERNAL ERROR: Merging Controller lists reveals duplicates during finalize_ecus!")

            tmp_data[ecu_name] = (ecu_data[0], tmp_ctrllist)

        for ecu_name, ecu_data in tmp_data.items():
            self.__ecus__[ecu_name] = self.__conf_factory__.create_ecu(
                ecu_name, ecu_data[1])
    ...
    def parse_utilization(self, element):
        ...
        if element is not None:
            coding = self.get_child_attribute(
                element, 'fx:UTILIZATION/fx:CODING-REF', 'ID-REF')
            ret["Coding"] = self.get_from_dict_or_none(
                self.__codings__, coding)

            high_low_byte_order = element.find(
                'fx:UTILIZATION/fx:IS-HIGH-LOW-BYTE-ORDER', self.__ns__)
            if high_low_byte_order is not None:
                high_low_byte_order = (
                    'true' == high_low_byte_order.text.lower())
    ...
    def merge_utilizations(self, util1, util2):
        ...
        if "Coding" in util1 and util1["Coding"] is not None and \
            ...
            ret["Coding"] = self.merge_utilizations(
                util1["Coding"], util2["Coding"])
    ...
    def parse_coding(self, element):
        ...
        if cs is not None:
            ...
            if len(compu_scale) != 2:
                print(
                    f"WARNING: We did not find to nums in the compu-numerator but {len(compu_scale)}!")
    ...
    def parse_signal(self, element):
        ...
        oid = self.get_oid(element)
        name = self.get_child_text(element, './ho:SHORT-NAME')
        code_id = self.get_child_attribute(
            element, './fx:CODING-REF', 'ID-REF')
    ...
    def parse_signal_instance(self, element):
        ...
        if bit_position is None:
            ...
            bit_position = int(self.prev_bit_position_parse_pdus)

        is_high_low_byte_order = self.get_child_text(
            element, './fx:IS-HIGH-LOW-BYTE-ORDER')
        signal_ref = self.get_child_attribute(
            element, './fx:SIGNAL-REF', 'ID-REF')

        ret = self.__conf_factory__.create_signal_instance(
            id, signal_ref,
            bit_position,
    ...
    def parse_multiplexer(self, element):
        ...
        bit_pos = int(self.get_child_text(
            element, './fx:SWITCH/fx:BIT-POSITION'))
        is_high_low_byte_order = self.get_child_text(
            element, './fx:SWITCH/fx:IS-HIGH-LOW-BYTE-ORDER')
        bit_length = int(self.get_child_text(
            element, './fx:SWITCH/ho:BIT-LENGTH'))
        switch = self.__conf_factory__.create_multiplex_switch(
            id, name, bit_pos, is_high_low_byte_order, bit_length)
        ...
        for seg in element.findall('./fx:DYNAMIC-PART/fx:SEGMENT-POSITIONS/fx:SEGMENT-POSITION', self.__ns__):
            ...
            segs.append(self.__conf_factory__.create_multiplex_segment_position(
                bit_pos, high_low, bit_len))
        ...
        for switched_pdu in element.findall('./fx:DYNAMIC-PART/fx:SWITCHED-PDU-INSTANCES/fx:SWITCHED-PDU-INSTANCE',
            ...
            pdu_ref = self.get_child_attribute(
                switched_pdu, './fx:PDU-REF', 'ID-REF')
        ...
        for seg in element.findall('./fx:STATIC-PART/fx:SEGMENT-POSITIONS/fx:SEGMENT-POSITION', self.__ns__):
            ...
            high_low = self.get_child_text(seg, './fx:IS-HIGH-LOW-BYTE-ORDER')
            bit_len = int(self.get_child_text(seg, './ho:BIT-LENGTH'))
            static_segs.append(self.__conf_factory__.create_multiplex_segment_position(
                bit_pos, high_low, bit_len))

        # static pdu instances
        static_pdu = self.get_child_attribute(
            element, './fx:STATIC-PART/fx:STATIC-PDU-INSTANCE/fx:PDU-REF', 'ID-REF')
    ...
    def parse_signal_pdu(self, element, verbose):
        ...
        if verbose:
            print(
                f"DEBUG: parse_pdu: {short_name} byte_length:{byte_length} pdu_type:{pdu_type}")
        ...
        for signal_instance in element.findall('./fx:SIGNAL-INSTANCES/fx:SIGNAL-INSTANCE', self.__ns__):
            ...
            added_quant = int(byte_length)*8-1 if int(byte_length) > 0 else 0
            self.prev_bit_position_parse_pdus += added_quant
            si.add_signal(self.get_signal(si.__signal_ref__))
            if si is not None:
                signal_instances[si.__id__] = si

        ret = self.__conf_factory__.create_pdu(
            id, short_name, byte_length, pdu_type, signal_instances)
        self.add_pdu(ret)
        return ret
    ...
    def parse_multiplex_pdu(self, element, verbose):
        ...
        switch, seg_pos, pdu_instances, static_segs, static_pdu_id = self.parse_multiplexer(
            multiplexer)
    ...
    def parse_pdus(self, root, verbose):
        ...
        for pdu in root.findall('.//fx:PDUS/fx:PDU', self.__ns__):
            if pdu.find('./fx:MULTIPLEXER', self.__ns__) is None:
                self.prev_bit_position_parse_pdus = 0
        ...
        for pdu in root.findall('.//fx:PDUS/fx:PDU/fx:MULTIPLEXER/..', self.__ns__):
            # self.prev_bit_position_multiplex_parse_pdus = 0
    ...
    def parse_pdu_instance(self, element):
        ...
        req_bit_position = self.get_child_text(element, './fx:BIT-POSITION')
        if req_bit_position is None:

            bit_position = int(self.prev_bit_position_parse_frames)

        is_high_low_byte_order = self.get_child_text(
            element, './fx:IS-HIGH-LOW-BYTE-ORDER')
        pdu_update_bit_position = self.get_child_text(
            element, './/fx:PDU-UPDATE-BIT-POSITION')
        if pdu_update_bit_position is not None:
            pdu_update_bit_position = int(pdu_update_bit_position)

        ret = self.__conf_factory__.create_pdu_instance(
            id, pdu_ref, bit_position,
            is_high_low_byte_order, pdu_update_bit_position)
        return ret

    def parse_frame_triggering(self, element):
        id = self.get_id(element)
        frame_ref = self.get_child_attribute(
            element, './fx:FRAME-REF', 'ID-REF')
        frame = self.__frames__.get(frame_ref, None)

        # let us find out what we have here...

        # CAN:
        identifier_tmp = self.get_child_text(
            element, './fx:IDENTIFIER/fx:IDENTIFIER-VALUE')

        # FlexRay
        slot_id_tmp = self.get_child_text(
            element, './fx:TIMINGS/fx:ABSOLUTELY-SCHEDULED-TIMING/fx:SLOT-ID')
        cycle_counter_tmp = self.get_child_text(
            element, './fx:TIMINGS/fx:ABSOLUTELY-SCHEDULED-TIMING/fx:CYCLE-COUNTER')
        base_cycle_tmp = self.get_child_text(
            element, './fx:TIMINGS/fx:ABSOLUTELY-SCHEDULED-TIMING/fx:BASE-CYCLE')
        cycle_repetition_tmp = self.get_child_text(
            element, './fx:TIMINGS/fx:ABSOLUTELY-SCHEDULED-TIMING/fx:CYCLE-REPETITION')
        ...
        if slot_id_tmp is not None and ((cycle_counter_tmp is not None) or
            ...
            cycle_counter = None if cycle_counter_tmp is None else int(
                cycle_counter_tmp)
            base_cycle = None if base_cycle_tmp is None else int(
                base_cycle_tmp)
            cycle_repetition = None if cycle_repetition_tmp is None else int(
                cycle_repetition_tmp)
            ...
            ret = self.__conf_factory__.create_frame_triggering_can(
                id, frame, can_id)
            return ret
        ...
        for frame_triggering in root.findall('.//fx:FRAME-TRIGGERING', self.__ns__):
            ...
            if f is not None:
                if f.id() in self.__frame_triggerings__.keys():
                    print(
                        f"WARNING: creating another Frame Triggering with ID: {f.id()}")
    ...
    def parse_frame(self, element, verbose):
        ...
        for pdu_instance in element.findall('./fx:PDU-INSTANCES/fx:PDU-INSTANCE', self.__ns__):
            added_quant = int(byte_length)*8-1 if int(byte_length) > 0 else 0
            self.prev_bit_position_parse_frames += added_quant
            pi = self.parse_pdu_instance(pdu_instance)
            if pi is not None:
                pdu = self.get_pdu(pi.__pdu_ref__)
                if pdu is None:
                    print(
                        f"ERROR: Frame {short_name} references unknown PDU {pi.__pdu_ref__}!")
                else:
                    pi.add_pdu(pdu)

                pdu_instances[pi.__id__] = pi

        ret = self.__conf_factory__.create_frame(
            id, short_name, byte_length, frame_type, pdu_instances)
        return ret

    def parse_frames(self, root, verbose):
        for frame in root.findall('.//fx:FRAMES/fx:FRAME', self.__ns__):
            self.prev_bit_position_parse_frames = 0
    ...
    def interpret_datatype(self, element, utils, serialization_attributes):
        ...
        if coding_ref is not None:
            coding2 = self.__codings__[
                self.get_attribute(coding_ref, 'ID-REF')]
        ...
        if p["Type"] == "fx:COMMON-DATATYPE-TYPE" or p["Type"] == "fx:ENUM-DATATYPE-TYPE":
            if self.basetype_is_int(coding2) or self. basetype_is_float(coding2):
                ...
                if maxlen == -1:
                    ...
                    if maxlen != -1:
                        print(
                            "WARNING: FIBEX seems strange. I found a MaxLength in Coding!")
                ...
                if cat == "LEADING-LENGTH-INFO-TYPE":
                    lenoflen = self.get_from_dict(
                        serialization_attributes, "LengthFieldSize", 32)
            ...
            if p["Type"] == "fx:ENUM-DATATYPE-TYPE":
                ...
                for i in element.findall('./fx:ENUMERATION-ELEMENTS/fx:ENUM-ELEMENT', self.__ns__):
                    ...
                    items[value] = self.__conf_factory__.create_someip_parameter_enumeration_item(
                        value, name, desc)
        ...
        if p["Type"] == "fx:COMPLEX-DATATYPE-TYPE":
            p["ComplexClass"] = self.get_child_text(
                element, 'fx:COMPLEX-DATATYPE-CLASS')
            ...
            if "Members" in p:
                if self.get_from_dict_or_none(p, "ComplexClass") == "STRUCTURE":
                    ...
                    for m in p["Members"]:
                        ...
                        if "Array" in m and m["Array"] is not None:
                            child = self.build_array(
                                m["Name"],
                                self.get_from_dict(
                                    serialization_attributes, "ArrayLengthSize", -1),
                                m["Array"],
                                child
                            )
                    ...
                    len_of_len = self.get_from_dict(
                        serialization_attributes, "LengthFieldSize", 0)
                    padto = 0
                    ret = self.__conf_factory__.create_someip_parameter_struct(
                        p["Name"], len_of_len, padto, members)
                    ...
                    len_of_len = self.get_from_dict(
                        serialization_attributes, "LengthFieldSize", 32)
                    len_of_type = self.get_from_dict(
                        serialization_attributes, "TypeFieldSize", 32)
                    padto = 0
                    ...
                    for m in p["Members"]:
                        ...
                        if "Position" in m and m["Position"] == 0:
                            ...
                            child = self.interpret_datatype(
                                self.get_from_dict_or_none(self.__datatypes__,
                                                           self.get_from_dict_or_none(m, "DatatypeRef")),
                                self.merge_utilizations(
                                    utils, self.get_from_dict_or_none(m, "Utilization")),
                                self.get_from_dict_or_none(
                                    m, "SerializationAttributes")
                            )
                        ...
                        ret = self.__conf_factory__.create_someip_parameter_typedef(
                            p["Name"], childname, child)
        ...
        if ret is None:
            print("WARNING: interpretDatatype(%s, %s, %s, %s, %s, %s) returns None!!!!!!!" %
                  (element, utils, serialization_attributes,
                   p["ID"], p["Name"], p["Type"])
                  )
    ...
    def parse_array(self, element):
        ...
        for dimension in element.findall('./fx:ARRAY-DECLARATION/fx:ARRAY-DIMENSION', self.__ns__):
            if ret is None:
                ret = dict()
            minsize = self.element_text_to_int(
                dimension.find('fx:MINIMUM-SIZE', self.__ns__), 0)
            maxsize = self.element_text_to_int(
                dimension.find('fx:MAXIMUM-SIZE', self.__ns__), -1)
            dim = self.element_text_to_int(
                dimension.find('fx:DIMENSION', self.__ns__), -1)
            bit = self.element_text_to_int(
                dimension.find('fx:BIT-ALIGNMENT', self.__ns__), 0)

            if dim > 0:
                ret[dim] = {'dim': dim, 'max': maxsize,
                            'min': minsize, 'bitalignment': bit}
            else:
    ...
    def parse_parameter(self, param):
        ...
        p["Mandatory"] = self.element_text(
            param.find('./service:MANDATORY', self.__ns__))
        p["Position"] = self.element_text(
            param.find('./service:POSITION', self.__ns__))
        ...
        if "Array" in p and p["Array"] is not None:
            ret = self.build_array(
                p["Name"], serialization_attributes["ArrayLengthSize"], p["Array"], ret)
    ...
    def parse_method(self, element):
        id = self.get_id(element)
        name = self.get_child_text(element, './ho:SHORT-NAME')
        method_id = int(self.get_child_text(
            element, './service:METHOD-IDENTIFIER'))

        reliable = ("true" == self.get_child_text(
            element, './service:RELIABLE'))
    ...
    def parse_field(self, element):
        ...
        getter_id = (self.get_child_text(
            element, './service:GETTER/service:METHOD-IDENTIFIER'))
        setter_id = (self.get_child_text(
            element, './service:SETTER/service:METHOD-IDENTIFIER'))
        notifier_id = (self.get_child_text(
            element, './service:NOTIFIER/service:NOTIFICATION-IDENTIFIER'))
        ...
        getter_reli = ("true" == self.get_child_text(
            element, './service:GETTER/service:RELIABLE'))
        setter_reli = ("true" == self.get_child_text(
            element, './service:SETTER/service:RELIABLE'))
        notifier_reli = ("true" == self.get_child_text(
            element, './service:NOTIFIER/service:RELIABLE'))
        ...
        params = []
        child = self.interpret_datatype(
            datatype, utils, serialization_attributes)
        params += [self.__conf_factory__.create_someip_parameter(
            0, 'fieldparam', '', True, child, signal)]
    ...
    def parse_eventgroup(self, element, serviceid, events, fields):
        ...
        for eventref in element.findall('./service:EVENT-REFS/service:EVENT-REF', self.__ns__):
            ...
            if events is not None and ref in events:
                ...
                print(
                    f"ERROR: Eventgroup {id} has EVENT-REF to {ref} but I cannot find the Event!")
        ...
        for fieldref in element.findall('./service:FIELD-REFS/service:FIELD-REF', self.__ns__):
            ...
            if fields is not None and ref in fields:
                ...
                print(
                    "ERROR: Eventgroup %s has FIELD-REF to %s but I cannot find the Field!" % (id, ref))
    ...
    def parse_service(self, service):
        ...
        service_id = int(self.get_child_text(
            service, './fx:SERVICE-IDENTIFIER'))
        major_version = int(self.get_child_text(
            service, './service:API-VERSION/service:MAJOR'))
        minor_version = int(self.get_child_text(
            service, './service:API-VERSION/service:MINOR'))
    ...
    def parse_member(self, element, t):
        ...
        p["DatatypeRef"] = self.get_child_attribute(
            element, 'fx:DATATYPE-REF', 'ID-REF')
        p["SignalRef"] = self.get_child_attribute(
            element, 'fx:SIGNAL-REF', 'ID-REF')

        p["Index"] = self.element_text_to_int(
            element.find('fx:INDEX', self.__ns__), -1)
        ...
        p["SerializationAttributes"] = self.parse_serialization_attributes(
            element)
    ...
    def parse_channels(self, root):
        for ch in root.findall('.//fx:CHANNELS/fx:CHANNEL', self.__ns__):
            ...
            channel["flexray-channel-name"] = self.get_child_text(
                ch, "flexray:FLEXRAY-CHANNEL-NAME")
            ...
            for v in ch.findall('ethernet:VIRTUAL-LAN', self.__ns__):
                ...
                if channel["vlanid"] is not None:
                    print(
                        "WARNING: We have found a channel with more than 1 VLAN. We are skipping those.")
                else:
                    channel["vlanid"] = self.get_child_text(
                        v, "ethernet:VLAN-IDENTIFIER")
                    channel["vlanname"] = self.get_child_text(
                        v, "ho:SHORT-NAME")
    ...
    def parse_neps(self, element):
        ...
        for n in element.findall('it:NETWORK-ENDPOINTS/it:NETWORK-ENDPOINT', self.__ns__):
            ...
            nep["name"] = self.get_child_text(
                n, "it:MANUFACTURER-EXTENSION/ho:SHORT-NAME")
            ...
            for i in n.findall('it:NETWORK-ENDPOINT-ADDRESSES/it:NETWORK-ENDPOINT-ADDRESS/it:IPV4', self.__ns__):
                ...
                ip["addrsrc"] = self.get_child_text(
                    i, "it:IPV4-ADDRESS-SOURCE")
                ip["netmask"] = self.get_child_text(i, "it:NETWORKMASK")
                ipsv4 += [ip]

                if ip["addr"] is not None and ip["netmask"] is not None:
                    self.__conf_factory__.add_ipv4_address_config(
                        ip["addr"], ip["netmask"])
            nep["ipsv4"] = ipsv4
            ...
            for i in n.findall('it:NETWORK-ENDPOINT-ADDRESSES/it:NETWORK-ENDPOINT-ADDRESS/it:IPV6', self.__ns__):
                ...
                ip["addrsrc"] = self.get_child_text(
                    i, "it:IPV6-ADDRESS-SOURCE")
                ip["prefixlen"] = self.get_child_text(
                    i, "it:IP-ADDRESS-PREFIX-LENGTH")
                ipsv6 += [ip]

                if ip["addr"] is not None and ip["prefixlen"] is not None:
                    self.__conf_factory__.add_ipv6_address_config(
                        ip["addr"], ip["prefixlen"])
            nep["ipsv6"] = ipsv6
    ...
    def parse_psis(self, root):
        for aep in root.findall('.//it:APPLICATION-ENDPOINT', self.__ns__):
            protover = self.get_child_text(
                aep, 'it:SERIALIZATION-TECHNOLOGY/it:VERSION')
            if protover is None:
                protover = 1
            ...
            for psi in aep.findall('it:PROVIDED-SERVICE-INSTANCES/it:PROVIDED-SERVICE-INSTANCE', self.__ns__):
                ...
                servref = self.get_child_attribute(
                    psi, 'service:SERVICE-INTERFACE-REF', 'ID-REF')
                if servref not in self.__services__:
                    print(f"ERROR in FIBEX: I cannot find Service {servref}")
                else:
                    service = self.__services__[servref]

                    si = self.__conf_factory__.create_someip_service_instance(
                        service, instanceid, protover)
    ...
    def parse_csis_and_cegs(self, root):
        for aep in root.findall('.//it:APPLICATION-ENDPOINT', self.__ns__):
            ...
            for csi in aep.findall('it:CONSUMED-SERVICE-INSTANCES/it:CONSUMED-SERVICE-INSTANCE', self.__ns__):
                psiid = self.get_child_attribute(
                    csi, 'it:PROVIDED-SERVICE-INSTANCE-REF', 'ID-REF')
                ...
                if psiid in self.__ServiceInstances__:
                    ...
                    for ceg in csi.findall('it:CONSUMED-EVENT-GROUPS/it:CONSUMED-EVENT-GROUP', self.__ns__):
                        ...
                        egref = self.get_child_attribute(
                            ceg, 'service:EVENT-GROUP-REF', 'ID-REF')
                        aepref = self.get_child_attribute(
                            ceg, 'it:APPLICATION-ENDPOINT-REF', 'ID-REF')

                        if egref not in self.__eventgrouprefs__:
                            print(
                                f"ERROR in FIBEX: I cannot find Eventgroup {egref}!")

                        else:
                            egid = self.__eventgrouprefs__[egref][1]
                            tmp = self.__conf_factory__.create_someip_service_eventgroup_receiver(
                                si, egid, None)
                            ...
                            if cegid not in self.__ServiceEventgroupReceiver__.keys():
                                ...
                                print(
                                    f"ERROR in FIBEX: The CEG ID seems to be not unique {egid}!")
    ...
    def parse_generic_frame_triggering_ref(self, root, path, frametriggerings):
        ...
        for port in root.findall(path, self.__ns__):
            frame_triggering_id = self.get_child_attribute(
                port, "./fx:FRAME-TRIGGERING-REF", "ID-REF")

            tmp = self.__frame_triggerings__.get(frame_triggering_id, None)
            if tmp is None:
                print(
                    f"WARNING: FrameTriggering {frame_triggering_id} not found!")
            else:
    ...
    def parse_inputs_outputs(self, root, channel_fts):
        input_ports = self.parse_generic_frame_triggering_ref(
            root, "./fx:INPUTS/fx:INPUT-PORT", channel_fts)
        output_ports = self.parse_generic_frame_triggering_ref(
            root, "./fx:OUTPUTS/fx:OUTPUT-PORT", channel_fts)
    ...
    def parse_ecus(self, root):
        ...
        for e in root.findall('.//fx:ECUS/fx:ECU', self.__ns__):
            ...
            for c in e.findall('fx:CONNECTORS/fx:CONNECTOR', self.__ns__):
                channelref = self.get_child_attribute(
                    c, 'fx:CHANNEL-REF', 'ID-REF')
                ctrlref = self.get_child_attribute(
                    c, 'fx:CONTROLLER-REF', 'ID-REF')
                ...
                if channelref in self.__channels__:
                    ...
                    channel = None
                    print(
                        f"ERROR in FIBEX: I cannot find channel '{channelref}' (ID: {self.get_id(c)})")
                    channel_fts = {}

                input_frame_trigs, output_frame_trigs = self.parse_inputs_outputs(
                    c, channel_fts)
                ...
                for aep in c.findall('it:APPLICATION-ENDPOINTS/it:APPLICATION-ENDPOINT', self.__ns__):
                    ...
                    aep_name = self.get_child_text(
                        aep, 'it:MANUFACTURER-EXTENSION/ho:SHORT-NAME')
                    nepref = self.get_child_attribute(
                        aep, 'it:NETWORK-ENDPOINT-REF', 'ID-REF')
            ...
            for key in sorted(ctrls.keys()):
                ctrl = ctrls[key]
                tmp = self.__conf_factory__.create_controller(
                    ctrl["name"], ctrl["ifaces"])
                ctrllist += [tmp]
    ...
    def parse_topology(self, root, verbose=False):
        for e in root.findall('.//fx:COUPLING-ELEMENTS/fx:COUPLING-ELEMENT', self.__ns__):
            ...
            cluster_ref = self.get_child_attribute(
                e, "fx:CLUSTER-REF", "ID-REF")
            ecu_ref = self.get_child_attribute(e, "fx:ECU-REF", "ID-REF")
            coupling_element_type = self.get_child_text(
                e, "ethernet:COUPLING-ELEMENT-TYPE")
            ...
            if coupling_element_type != "SWITCH":
                print(
                    f"Found unsupported Coupling Element with coupling_element_type={coupling_element_type}!")
                continue

            coupling_ports = []
            for c in e.findall('fx:COUPLING-PORTS/fx:COUPLING-PORT', self.__ns__):
                coupling_port_id = self.get_attribute(c, "ID")
                controller_ref = self.get_child_attribute(
                    c, "fx:CONTROLLER-REF", "ID-REF")
                ...
                coupling_port_ref = self.get_child_attribute(
                    c, "fx:COUPLING-PORT-REF", "ID-REF")
                coupling_port = self.__coupling_ports__.get(
                    coupling_port_ref, None)

                default_vlan_ref = self.get_child_attribute(
                    c, "ethernet:DEFAULT-VLAN/fx:CHANNEL-REF", "ID-REF")

                if verbose:
                    default_vlan_name = (self.__channels__.get(
                        default_vlan_ref, {})).get("name", "")
                ...
                for v in c.findall('ethernet:VLAN-MEMBERSHIPS/ethernet:VLAN-MEMBERSHIP', self.__ns__):
                    channel_ref = self.get_child_attribute(
                        v, "fx:CHANNEL-REF", "ID-REF")
                    channel_ref_name = (self.__channels__.get(
                        channel_ref, {})).get("name", "")
                    default_prio = self.get_child_text(
                        v, "ethernet:DEFAULT-PRIORITY/fx:PRIORITY")
                    default_prio = 0 if default_prio is None else int(
                        default_prio)

                    if verbose:
                        print(
                            f"    VLAN Channel:{channel_ref} ({channel_ref_name}) Default-Prio:{default_prio}")

                    channel = self.__channels__.get(channel_ref, {})
                    channel["vlanid"] = None if channel.get(
                        "vlanid", None) is None else int(channel["vlanid"])
                    vlans.append(self.__conf_factory__.create_vlan(
                        channel["name"], channel["vlanid"], default_prio))
            ...
            self.__conf_factory__.create_switch(
                switch_name, ecu, coupling_ports)
