...
from ..DecoderBase import DecoderBase
...
class DataFrameDecoder(DecoderBase, metaclass=ABCMeta):
    ...
    def __new__(cls, conversion_rules: SignalDB, *args, **kwargs):
        ...
        if result is None:
            raise ValueError(
                "No known support for protocol: \"{}\"".format(dbc_protocol))
    ...
    def _get_fused_ids(cls, df: pd.DataFrame) -> np.ndarray:
        ...
        id_column = df.get('CAN_DataFrame.CAN_DataFrame.ID')
        # type: Optional[pd.Series]
        ide_column = df.get('CAN_DataFrame.CAN_DataFrame.IDE')
    ...
    def _add_series(self, df: pd.DataFrame) -> None:
        ...
        columns_to_drop_in_frame = self._columns_to_drop.intersection(
            columns_in_frame)
    ...
    def decode_frame(self, df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
        ...
        * **DataBytes** - A Python list of integers, where each integer has the value of the corresponding byte in the
        ...
        if (
                # ("ID" not in columns)
                # or
                ('CAN_DataFrame.CAN_DataFrame.ID' not in columns)):
            raise ValueError("Missing ID column in input data")
        elif (
            #   ("IDE" not in columns)
            # or
                ('CAN_DataFrame.CAN_DataFrame.IDE' not in columns)):
            raise ValueError("Missing IDE column in input data")
        elif (
            #   ("DataBytes" not in columns)
            # or
                ('CAN_DataFrame.CAN_DataFrame.DataBytes' not in columns)):
