...
class HDF5Decoder():
    def __init__(self, verbose=True, use_attrdict=False,
        ...
        if only_include is not None:
            only_include = [s if s[0] ==
                            '/' else f'/{s}' for s in only_include]
            only_include = [s[:-1] if s[-1] ==
                            '/' else s for s in only_include]
            ...
            for key in hdf5:
                ...
                if MATLAB_class == 'struct' and len(elem) > 1 and \
                    ...
                    if allist and same_len:
                        ...
                        keys = unpacked.keys()
                        struct = [{k: v for v, k in zip(
                            row, keys)} for row in items]
    ...
    def convert_mat(self, dataset, depth, MATLAB_class=None):
        ...
        if mtype == 'cell':
            ...
            for ref in dataset:
                ...
                if isinstance(ref, Iterable):
                    for r in ref:
                        ...
                        entry = self.unpack_mat(
                            self.refs.get(r), depth+1, force=True)
                        row.append(entry)
                else:
                    row = [ref]
                cell.append(row)
            ...
            char_arr = np.apply_along_axis(
                lambda x: ''.join(x), axis=char_axis, arr=arr)
